...
in {
  options.vim.visuals = {
    ...
    highlight = {
      enable = mkOption {
        type = types.bool;
        description = "Enable nvim-cursorline";
      };
      cursorWord = {
        enable = mkOption {
          type = types.bool;
          description = "enable word highlight [nvim-cursorline]";
        };

        minLenght = mkOption {
          type = types.int;
          description = "Min lenght for cursorword to appear";
        };

        hlUnderline = mkOption {
          type = types.bool;
          description = "enable underline highlight [nvim-cursorline]";
        };
      };
  ...
  config =
    mkIf cfg.enable
    {
      vim.startPlugins = [
        ...
        (
          if cfg.lspkind.enable then "lspkind" else null
        )
        (
          if cfg.highlight.enable then "nvim-cursorline" else null
        )
        (
        ...
        ${
          ...
          then ''
              require('nvim-cursorline').setup {
                --cursorline = {
                --  enable  = ${toString cfg.highlight.cursorWordLine.enable},
                --  timeout = ${toString cfg.highlight.cursorWordLine.lineTimeout},
                --  number = false,
                --},
                cursorword = {
                  enable      = ${toString cfg.highlight.cursorWord.enable},
                  min_length  = ${toString cfg.highlight.cursorWord.minLenght},
                  hl = {
